    ...
    <div className="mx-auto flex h-[calc(100vh-7rem)] w-full max-w-6xl flex-col gap-6 px-6 pt-8 pb-0">
      ...
      {projects.length === 0 ? (
        ...
        <div className="relative min-h-0 flex-1 pb-14">
            ...
            {pagedProjects.map((p) => (
              <li key={p.id}>
                <Link
                  to={`/projects/${p.id}`}
                  className={cn(
                    "group block aspect-square rounded-lg border border-border/70 bg-card p-3 transition-colors",
                  ...
                  <div className="flex h-full flex-col">
                    ...
                    <div className="mt-3 space-y-1 text-[11px] text-muted-foreground">
                      <p className="truncate">
                        {p.storageType === "remote" ? `remote://${p.remoteIp}:${p.remotePort}` : p.localPath}
                      </p>
          ...
          <div className="absolute right-0 bottom-0 left-0 translate-y-1/2 rounded-lg border border-border/60 bg-background/90 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80">
